...
//
//          Basic matrix class (real precision)
//
...
using std::endl;
  ...
  int* MV_ColMat::pivotPtr(){return pivots;} //added by cek
  bool MV_ColMat::apply(const Vec& x, Vec& Ax)
  {
    ...
    unsigned int            MV_ColMat::lda(void) const{ return lda_; }
    unsigned int            MV_ColMat::size(int i) const { return dim(i);} 
    int MV_ColMat::ref() const { return ref_;}
  void MV_ColMat::setSize(int m, int n){newsize(m,n);}
  void MV_ColMat::Solve(MV_Vector, MV_Vector){}
  //unc_sim functions
  int MV_ColMat::rowNum(){return dim(0);}
  int MV_ColMat::colNum(){return dim(1);}

  //extensions for wrappers
  real* MV_ColMat::castToArray(){return v_.castToArray();}
  const real* MV_ColMat::castToConstArray() const {return v_.castToConstArray();}

unsigned int MV_ColMat::dim(int i) const 
{
    if (i==0) return dim0_;
    if (i==1) return dim1_;
    else
...
MV_ColMat::MV_ColMat()  : LinearOperator(0,0), v_(), dim0_(0), dim1_(0) , lda_(0),  ref_(0), pivots(0) {}
                                                                

MV_ColMat::MV_ColMat(unsigned int m, unsigned int n) : LinearOperator(m,n),v_(m*n),
        dim0_(m), dim1_(n), lda_(m), ref_(0) {pivots = new int[m];}

MV_ColMat::MV_ColMat(unsigned int m, unsigned int n, const real &s) : LinearOperator(m,n),v_(m*n),
...
MV_ColMat& MV_ColMat::operator=(const real & s) 
{
    int M = dim(0);
    int N = dim(1);
...
MV_ColMat& MV_ColMat::newsize(unsigned int M, unsigned int N)
{
    v_.newsize(M*N);
    dim0_ = M;
    dim1_ = N;
...
MV_ColMat& MV_ColMat::operator=(const MV_ColMat & m) 
{
...
MV_ColMat::MV_ColMat(const MV_ColMat & m) : LinearOperator(m),
...
MV_ColMat::MV_ColMat(real* d, unsigned int m, unsigned int n) : LinearOperator(m,n),
...
MV_ColMat::MV_ColMat(real* d, unsigned int m, unsigned int n, 
                     unsigned int lda) :LinearOperator(m,n),
...
MV_ColMat::~MV_ColMat() {delete [] pivots;}

ostream&   operator<<(ostream& s, const MV_ColMat& V)
{
    int M = V.dim(0);
    int N = V.dim(1);
    for (int i=0; i<M; i++)
