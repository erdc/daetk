#include "SaturatedPsk2p.h"

namespace Daetk
{

  SaturatedPsk2p::SaturatedPsk2p():
    Psk2p()
  {
    Tracer tr("SaturatedPsk2p()");
  }
  SaturatedPsk2p::~SaturatedPsk2p(){}
  void SaturatedPsk2p::readParameters(ParameterDatabase& pd)
  {
    Psk2p::readParameters(pd);
  }

  void SaturatedPsk2p::readZones(ParameterDatabase& pd,Petsc::SecondOrderFd& node)
  {
  ...
  void SaturatedPsk2p::millerSimilarScaling(Vec& delta)
  {
  ...
  void SaturatedPsk2p::setHeads(const Vec& psiW_vec, const Vec& psiN_vec)
  {
  ...
  void SaturatedPsk2p::setVFraction(const Vec& thetaW_vec)
  {
  ...
  void SaturatedPsk2p::calculateDerivativesHead(const Vec& psiW_vec, const Vec& psiN_vec)
  {
    for(int i=0;i<psiW_vec.ldim();i++)
      {
        setHeads(i,psiW_vec[i],psiN_vec[i]);
  ...
  void SaturatedPsk2p::calculateDerivativesVFraction(const Vec& thetaW_vec)
  {
    for(int i=0;i<thetaW_vec.ldim();i++)
      {
        setVFraction(thetaW_vec[i],i);
