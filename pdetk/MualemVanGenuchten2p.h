#ifndef MUALEMVANGENUCHTEN2P_H
#define MUALEMVANGENUCHTEN2P_H
...
namespace Daetk 
{
class MualemVanGenuchten2p : public Psk2p
{
public:
  MualemVanGenuchten2p();
  virtual ~MualemVanGenuchten2p();
  ...
  friend class Psk2pSpline<MualemVanGenuchten2p>;
protected:
  real vBar,uBar,
    alphaPsiC, alphaPsiC_n, alphaPsiC_nM1, alphaPsiC_nM2,
    onePlus_alphaPsiC_n,
    sqrt_sBar, sqrt_1minusSbar,
    sBarByOnePlus_alphaPsiC_n, sBarBy_onePlus_alphaPsiC_n_2,ns_del;
  Vec global_n,global_m,global_alpha;
  Vec n,m,alpha;
};
  
typedef Psk2pSpline<MualemVanGenuchten2p> MualemVanGenuchten2pSplineTest;
  
class MualemVanGenuchten2pSplineOld : public  MualemVanGenuchten2p
{
public:
  virtual ~MualemVanGenuchten2pSplineOld();
  MualemVanGenuchten2pSplineOld();
  void readParameters(ParameterDatabase& pd);
...
inline void MualemVanGenuchten2p::setHeads(int node,real psiWIn, real psiNIn)
{
  psiC = psiNIn - psiWIn;
  i = node; //local node#
...
inline void MualemVanGenuchten2p::setVFraction(const real& thetaWIn, int node)
{
  i = node;
...
inline void MualemVanGenuchten2p::calculateDerivatives()
{
...
inline void MualemVanGenuchten2pSplineOld::setHeads(int node,real psiWIn, real psiNIn)
{
  ...
  if (psiC < splineMinPsiC || psiC > splineMaxPsiC)
    MualemVanGenuchten2p::setHeads(node,psiWIn,psiNIn);
}

inline void MualemVanGenuchten2pSplineOld::setVFraction(const real& thetaWIn, int node)
{
  thetaW = thetaWIn;
  ...
  if (thetaW <= splineMinThetaW || thetaW >= splineMaxThetaW)
    MualemVanGenuchten2p::setVFraction(thetaW,node);
}

inline void MualemVanGenuchten2pSplineOld::calculateDerivatives()
{
...
inline void MualemVanGenuchten2pSplineOld::calculateDerivativesVFraction()
{
