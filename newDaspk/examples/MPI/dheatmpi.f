...
C
      ...
      parameter (Npara=24)
      PARAMETER (MAXM = 50, MAXM2 = MAXM+2, MXNEQ=npara*MAXM2*MAXM2)
...
*      DIMENSION INFO(30), RPAR(2), IPAR(4)
      include "mpif.h"
      character*30 procname
      real*4   t1, t2, t3, tt(2), secnds
      common /mpiinfo/myid, numprocs
c
c === CPU time set-up
c
      call MPI_INIT(IERR)
      ...
      call MPI_Get_processor_name(procname, lenget, ierr)

      if (myid.eq.0)PRINT *, ' Number of procs # ', numprocs
      print *, ' Processor # ', myid, ' on ', procname
...
c---------------------------------------------------------------
      ...
      info(inso+2) = 0          ! 0, central, 1, forward, 2 supply
      ...
      info(inso+8) = myid
      info(inso+9) = numprocs
 25   continue
C
C Call subroutine UINIT to initialize U and UPRIME.
C
      CALL UINIT (U, UPRIME, RPAR, IPAR, info(inso+1))
...
C------  End of main program for DHEAT example program -----------------
      ...
      T2 = MPI_WTIME() - T1
      IF (MYID .EQ. 0) THEN
         write(*,*) '   Time it took:',t2
      END IF
      call MPI_FINALIZE(IERR)
      STOP
      END

      SUBROUTINE UINIT (U, UPRIME, RPAR, IPAR, ISENFO)
C
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION U(*), UPRIME(*), RPAR(*), IPAR(*), ISENFO(*)
C
      NEQ = IPAR(3)
      np  = ipar(5)
      M = IPAR(4)
...
C Set problem constants using IPAR and RPAR.
      ...
      M2 = M + 2
      NAVG = np/NUMPROCS
      IF (MYID .LT. MOD(np, NUMPROCS)) THEN
         NPara = NAVG + 1
      ELSE
         NPara = NAVG
      END IF
